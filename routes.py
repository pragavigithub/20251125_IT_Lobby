...
def login():
    ...
    try:
        branches = db.session.execute(db.text(
            "SELECT branch_code as id, branch_name as name FROM branches WHERE is_active = TRUE ORDER BY branch_name")).fetchall()
    except Exception as e:
...
def user_management():
    ...
    try:
        branches = db.session.execute(
            db.text("SELECT id, name FROM branches WHERE is_active = TRUE ORDER BY name")).fetchall()
    except Exception as e:
...
def create_user():
    ...
    permissions = {}
    for screen in ['dashboard', 'grpo', 'inventory_transfer', 'serial_item_transfer', 'serial_transfer', 'user_management',
                   'qc_dashboard', 'invoice_creation', 'so_against_invoice']:
...
# ================================
# Warehouse Assignment Admin Routes
# ================================

@app.route('/admin/warehouse-assignments', methods=['GET'])
@login_required
def warehouse_assignments():
    """Admin page to manage user warehouse assignments"""
    if current_user.role != 'admin':
        flash('Access denied - Admin permissions required', 'error')
        return redirect(url_for('dashboard'))
    
    users = User.query.filter_by(is_active=True).order_by(User.username).all()
    return render_template('admin/warehouse_assignments.html', users=users)


@app.route('/api/admin/warehouses/fetch', methods=['GET'])
@login_required
def fetch_all_warehouses():
    """Fetch all warehouses from SAP B1 for admin assignment"""
    if current_user.role != 'admin':
    ...
    try:
        ...
        if sap.ensure_logged_in():
            url = f"{sap.base_url}/b1s/v1/Warehouses?$select=WarehouseCode,WarehouseName"
            headers = {
                'Content-Type': 'application/json',
                'Prefer': 'odata.maxpagesize=0'
            }
            ...
            if response.status_code == 200:
                data = response.json()
                warehouses = data.get('value', [])
                logging.info(f"✅ Fetched {len(warehouses)} warehouses from SAP B1 for admin")
                return jsonify({
                    'success': True,
                    'warehouses': warehouses
                })
            else:
                logging.error(f"❌ Failed to fetch warehouses: {response.status_code}")
                return jsonify({'success': False, 'error': 'Failed to fetch warehouses from SAP B1'}), 500
        else:
...
@app.route('/api/admin/warehouse-assignments/<int:user_id>', methods=['GET'])
@login_required
def get_user_warehouse_assignments(user_id):
    ...
    try:
        user = User.query.get_or_404(user_id)
        assignments = UserWarehouseAssignment.query.filter_by(user_id=user_id, is_active=True).all()
        ...
        for assignment in assignments:
            ...
            if assignment.assignment_type == 'from':
                from_warehouses.append(warehouse_data)
            else:
                to_warehouses.append(warehouse_data)
        ...
        logging.error(f"❌ Error getting user warehouse assignments: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/admin/warehouse-assignments/<int:user_id>', methods=['POST'])
@login_required
def save_user_warehouse_assignments(user_id):
    """Save warehouse assignments for a user (Admin only)"""
    # Security check: Only admins can manage warehouse assignments
    if current_user.role != 'admin':
        logging.warning(f"⚠️ Unauthorized warehouse assignment attempt by user {current_user.username} (role: {current_user.role})")
        return jsonify({'success': False, 'error': 'Admin access required'}), 403
    ...
    try:
        ...
        data = request.get_json()
        if data is None:
            logging.error("❌ No JSON data received in warehouse assignment request")
            return jsonify({'success': False, 'error': 'Invalid request: No data provided'}), 400
        
        from_warehouses = data.get('from_warehouses', [])
        to_warehouses = data.get('to_warehouses', [])
        
        # Validate data types
        if not isinstance(from_warehouses, list) or not isinstance(to_warehouses, list):
            logging.error("❌ Invalid data format for warehouse assignments")
            return jsonify({'success': False, 'error': 'Invalid data format'}), 400
        
        # Validate warehouse data structure
        for warehouse_list in [from_warehouses, to_warehouses]:
            for warehouse in warehouse_list:
                if not isinstance(warehouse, dict):
                    return jsonify({'success': False, 'error': 'Invalid warehouse data structure'}), 400
                if 'warehouse_code' not in warehouse or 'warehouse_name' not in warehouse:
                    return jsonify({'success': False, 'error': 'Missing required warehouse fields'}), 400
        
        logging.info(f"📝 Updating warehouse assignments for user {user.username} - From: {len(from_warehouses)}, To: {len(to_warehouses)}")
        
        # Delete existing assignments
        existing_count = UserWarehouseAssignment.query.filter_by(user_id=user_id).delete()
        logging.info(f"🗑️ Deleted {existing_count} existing assignments")
        
        # Create new assignments for "from" warehouses
        for warehouse in from_warehouses:
            assignment = UserWarehouseAssignment(
                user_id=user_id,
                warehouse_code=warehouse['warehouse_code'],
                warehouse_name=warehouse['warehouse_name'],
                assignment_type='from',
                is_active=True
            )
            db.session.add(assignment)
        
        # Create new assignments for "to" warehouses
        for warehouse in to_warehouses:
            assignment = UserWarehouseAssignment(
                user_id=user_id,
                warehouse_code=warehouse['warehouse_code'],
                warehouse_name=warehouse['warehouse_name'],
        ...
        logging.info(f"✅ Warehouse assignments saved successfully for user {user.username}")
        
        return jsonify({
            'success': True,
            'message': f'Warehouse assignments updated successfully for {user.username}',
            'from_count': len(from_warehouses),
            'to_count': len(to_warehouses)
        })
        
    except Exception as e:
        db.session.rollback()
        logging.error(f"❌ Error saving warehouse assignments: {str(e)}")
        import traceback
        logging.error(traceback.format_exc())
        return jsonify({'success': False, 'error': f'Failed to save assignments: {str(e)}'}), 500


@app.route('/api/get-assigned-warehouses', methods=['GET'])
@login_required
def get_assigned_warehouses():
    """Get warehouses assigned to the current user (filtered by assignment type)"""
    try:
        assignment_type = request.args.get('type', 'from')  # 'from' or 'to'
        ...
        if current_user.role == 'admin':
            ...
            if sap.ensure_logged_in():
                ...
                if response.status_code == 200:
                    data = response.json()
                    warehouses = data.get('value', [])
                    logging.info(f"✅ Admin retrieved {len(warehouses)} warehouses from SAP B1")
                    return jsonify({
                        'success': True,
                        'warehouses': warehouses,
                        'is_admin': True
                    ...
                    logging.error(f"❌ Failed to fetch warehouses from SAP: {response.status_code}")
                    return jsonify({
                        'success': False,
                        'error': 'Failed to fetch warehouses from SAP B1',
                        'warehouses': []
                    }), 500
            else:
                logging.error("❌ SAP B1 connection failed")
                return jsonify({
                    'success': False,
                    'error': 'SAP B1 connection failed',
                    'warehouses': []
                }), 500
        
        # Regular users: Get only their assigned warehouses
        assignments = UserWarehouseAssignment.query.filter_by(
            user_id=current_user.id,
        ...
        for assignment in assignments:
            warehouses.append({
                'WarehouseCode': assignment.warehouse_code,
                'WarehouseName': assignment.warehouse_name
            })
        
        # Provide helpful feedback if no warehouses are assigned
        if not warehouses:
            logging.warning(f"⚠️ User {current_user.username} has no {assignment_type} warehouse assignments")
            return jsonify({
                'success': True,
                'warehouses': [],
                'is_admin': False,
                'message': f'No {assignment_type} warehouses assigned. Please contact your administrator.'
            })
        
        logging.info(f"✅ User {current_user.username} retrieved {len(warehouses)} assigned {assignment_type} warehouses")
        return jsonify({
            'success': True,
            'warehouses': warehouses,
            'is_admin': False
        ...
        logging.error(f"❌ Error getting assigned warehouses: {str(e)}")
        import traceback
        logging.error(traceback.format_exc())
        return jsonify({
            'success': False,
            'error': 'An error occurred while fetching warehouses. Please contact support.',
            'warehouses': []
        }), 500

...
# ================================
# User Warehouse Assignment Models
# ================================

class UserWarehouseAssignment(db.Model):
    """User Warehouse Assignment - Admin assigns from/to warehouses to users"""
    ...
    assignment_type = db.Column(db.String(10), nullable=False)  # 'from' or 'to'
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    user = db.relationship('User', backref='warehouse_assignments')
    
    def __repr__(self):
        return f'<UserWarehouseAssignment User={self.user_id} Warehouse={self.warehouse_code} Type={self.assignment_type}>'
